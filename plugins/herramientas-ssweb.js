...
try{
     await conn.sendMessage(m.chat, { image: { url: `https://files.catbox.moe/2txrtp.jpgpp }, caption: `Tu imagen 📷` }, { quoted: m })
}
catch{
try {
...
if (!user.premiumTime) {
calidad = '1280x720' //HD
webIMG = `https://files.catbox.moe/2txrtp.jpgpp
...
async function ssweb(url, device = 'desktop') {
     return new Promise((resolve, reject) => {
          const base = 'https://files.catbox.moe/2txrtp.jpgpp'
          const param = {
            url: url,
            device: device,
            cacheLimit: 0
